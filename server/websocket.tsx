...
function WebsocketTerminal ({setMessages}: {setMessages: React.Dispatch<SetStateAction<MessageInput[]>>}) {
  const ipServer = 'ws://192.168.1.52:8765';
  const { sendJsonMessage, lastMessage, lastJsonMessage, readyState } = useWebSocket(ipServer);
  ...
  const sendMessageToSocket = ({message, question}:{message:string, question:string}) => {
    const object = {
      type: 'setParam',
      message,
      question,
